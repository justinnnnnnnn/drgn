...
// Waveform 
// </div>
//   <input type="range" min="0" max="3" step="1" value="0" id="waveform-slider-input" oninput="updateWaveformSlider(this.value)">
//   </label>
//   </div>`;
var waveformSlider = `<div id="waveform-slider"><label><div class="font-control">
Waveform 
...
const updateWaveformSlider = (slideAmount) => {
  ...
  slider = slideAmount;
  for (const oscWave in oscillator) {
    oscillator[oscWave].type = WAVEFORMS[sliderValue]
  }
  return sliderValue = slider;
};
var sliderValue = document.querySelector("#waveform-slider-input[value]").value;
// const waveformEvent = document.getElementById("waveform-slider-input");
// waveformEvent.addEventListener('input', updateWaveformSlider(this.value));
// waveformEvent.addEventListener('input', console.log('sup'));
...
const adsrSlider = `<div class="sliders" id="adsr">
  <label><div class="font-control">Attack</div>
    <input type="range" min="1" max="120" step="1" value="1" id="adsr-a" oninput="updateAttackSlider(this.value)">
  </label>
  <label><div class="font-control">Decay</div>
    <input type="range" min="1" max="120" step="1" value="1" id="adsr-d" oninput="updateDecaySlider(this.value)">
  </label>
  <label><div class="font-control">Sustain</div>
    <input type="range" min="0" max="1" step="0.1" value="1" id="adsr-s" oninput="updateSustainSlider(this.value)">
  </label>
  <label><div class="font-control">Release</div>
    <input type="range" min="1" max="120" step="1" value="1" id="adsr-r" oninput="updateReleaseSlider(this.value)">
  </label>
  <label><div class="font-control">FM</div>
    <input type="range" min="1" max="120" step="1" value="1" id="FM" oninput="updateFM(this.value)">
  </label>
  <label><div class="font-control">Pre-Filter</div>
    <input type="range" min="1" max="120" step="1" value="120" id="filter" oninput="updateFilter(this.value)">
  </label>
  <label><div class="font-control">Pre-Filter Q</div>
    <input type="range" min="1" max="120" step="1" value="10" id="filterQ" oninput="updateFilterQ(this.value)">
  </label>
  <label><div class="font-control">Post-Filter</div>
    <input type="range" min="1" max="120" step="1" value="120" id="filter2" oninput="updateFilter2(this.value)">
  </label>
  <label><div class="font-control">Post-Filter Q</div>
    <input type="range" min="1" max="120" step="1" value="10" id="filterQ2" oninput="updateFilterQ2(this.value)">
  </label>
  <label><div class="font-control">Delay Time</div>
    <input type="range" min="1" max="120" step="1" value="1" id="delayTime" oninput="updateDelayTime(this.value)">
  </label>
  <label><div class="font-control">Delay Feedback</div>
    <input type="range" min="1" max="120" step="1" value="1" id="delayFeedback" oninput="updateDelayFeedback(this.value)">
  </label>
  <label><div class="font-control">Detune</div>
    <input type="range" min="0" max="0.02973" step="0.00003" value="0" id="detune" oninput="updateDetune(this.value)">
  </label>
  <label><div class="font-control">Distortion Gain</div>
    <input type="range" min="1" max="120" step="1" value="1" id="distortion" oninput="updateDistortion(this.value)">
  </label>
  <label><div class="font-control">Distortion Wave</div>
    <input type="range" min="0" max="1" step="0.01" value="0" id="distortionWave" oninput="updateDistortionWave(this.value)">
  </label>
</div>`;
...
var updateFM = (sliderVal) => {
  ...
  for (const ampFM in amplifier) {
    if (ampFM.startsWith("FM+")) {
      amplifier[ampFM].gain.value = FM
    }
  }
  console.log("FM", FM)
};
...
var updateDetune = (sliderVal) => {
  ...
  for (const oscDetune in oscillator) {
    if (oscDetune.startsWith("2osc")) {
      let noteFreq = oscillator[`osc+${oscDetune.substring(5)}`].frequency.value
      oscillator[oscDetune].frequency.value = uptune(noteFreq)
    } 
    if (oscDetune.startsWith("3osc")) {
      let noteFreq = oscillator[`osc+${oscDetune.substring(5)}`].frequency.value
      oscillator[oscDetune].frequency.value = downtune(noteFreq)
    }
  }
  console.log(DETUNE)
};
    ...
    `<div id="modal">
      ...
      <div class="modal-content">
        ...
        <div id="param-documentation">
          ...
          <div><b class="bold-div">Waveform</b> is the root sound of the oscillator: the positions are Sine, Triangle, Saw, and Square.</div>
